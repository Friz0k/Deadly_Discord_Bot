...
async def add_family(ctx, discord_id: int, *, nickname: str):
    ...
    conn.commit()
    await ctx.send(f'✅ <@{discord_id}> (`{nickname}`) добавлен в семью. (Отправил: {ctx.author.mention})')
...
async def remove_family(ctx, discord_id: int):
    ...
    conn.commit()
    await ctx.send(f'✅ <@{discord_id}> (`{nickname}`) удалён из семьи. (Отправил: {ctx.author.mention})')
...
async def add_car(ctx, nickname: str = None, model: str = None, plate: str = None):
    ...
    try:
        ...
        conn.commit()
        car_id = c.lastrowid
        await ctx.send(f'🚗 {model} ({plate}) добавлен, номер {car_id}. Владелец: `{nickname}`. (Отправил: {ctx.author.mention})')
...
async def remove_car(ctx, plate: str):
    ...
    conn.commit()
    await ctx.send(f'🗑️ Машина с госномером `{plate}` удалена. (Отправил: {ctx.author.mention})')
...
@commands.check(is_admin)
async def take_car(ctx, car_id: int, hours: float = 2.0, nickname: str = None):
    if nickname is None:
        nickname = get_family_nickname(ctx.author.id)
        if not nickname:
            return await ctx.send('❌ Вы не в семье. Укажите ник явно: `!взятьавто Номер Часы Ник`.')
    else:
        nickname = nickname.replace("_", " ")
    check_auto_return()
    ...
    conn.commit()
    await ctx.send(f'✅ Авто `{plate}` выдано `{nickname}` на {hours} ч до {return_at.strftime("%d.%m.%Y %H:%M")}. (Отправил: {ctx.author.mention})')
...
async def return_car(ctx, car_id: int):
    ...
    conn.commit()
    await ctx.send(f'✅ Авто `{plate}` возвращено. (Отправил: {ctx.author.mention})')
...
async def take_from_warehouse(ctx, nickname: str = None, item: str = None, amount: int = None):
    ...
    conn.commit()
    await ctx.send(f'✅ `{nickname}` забрал {amount} x {item} со склада. (Отправил: {ctx.author.mention})')
...
async def put_to_warehouse(ctx, nickname: str = None, item: str = None, amount: int = None):
    ...
    c.execute("INSERT INTO warehouse (item, amount) VALUES (?, ?) ON CONFLICT(item) DO UPDATE SET amount = amount + ?",
              (item, amount, amount))
    conn.commit()
    await ctx.send(f'✅ `{nickname}` положил {amount} x {item} на склад. (Отправил: {ctx.author.mention})')
...
async def bank_add(ctx, amount: int, *, reason: str = "Без причины"):
    ...
    msg = f'💰 Семейный счёт пополнен на {amount}. Причина: {reason}. Баланс семьи: {new_balance}. (Отправил: {ctx.author.mention})'
...
async def bank_add_target(ctx, nickname: str, amount: int, *, reason: str = "Без причины"):
    ...
    for att in ctx.message.attachments:
        if att.content_type and att.content_type.startswith("image/"):
            ...
            new_filename = att.filename.replace("_", "-")
            files.append(discord.File(fp=io.BytesIO(img_bytes), filename=new_filename))
    msg = f'💰 Семейный счёт пополнен на {amount} (для `{nickname}`). Причина: {reason}. Баланс семьи: {new_balance}. (Отправил: {ctx.author.mention})'
    await ctx.send(msg, files=files if files else None)
...
async def bank_remove(ctx, amount: int, *, reason: str = "Без причины"):
    ...
    if balance < amount:
        return await ctx.send(f'❌ Недостаточно средств в семейном бюджете. Баланс: {balance}.')
    c.execute("UPDATE bank SET balance = balance - ? WHERE id='family'", (amount,))
    conn.commit()
    new_balance = get_family_balance()
    files = []
    for att in ctx.message.attachments:
        if att.content_type and att.content_type.startswith("image/"):
            ...
            new_filename = att.filename.replace("_", "-")
            files.append(discord.File(fp=io.BytesIO(img_bytes), filename=new_filename))
    msg = f'💸 Из семейного бюджета снято {amount}. Причина: {reason}. Баланс семьи: {new_balance}. (Отправил: {ctx.author.mention})'
    await ctx.send(msg, files=files if files else None)
...
async def bank_remove_target(ctx, nickname: str, amount: int, *, reason: str = "Без причины"):
    ...
    for att in ctx.message.attachments:
        if att.content_type and att.content_type.startswith("image/"):
            ...
            new_filename = att.filename.replace("_", "-")
            files.append(discord.File(fp=io.BytesIO(img_bytes), filename=new_filename))
    msg = f'💸 Из семейного бюджета снято {amount} (для `{nickname}`). Причина: {reason}. Баланс семьи: {new_balance}. (Отправил: {ctx.author.mention})'
    await ctx.send(msg, files=files if files else None)
