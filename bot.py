...
async def add_family(interaction: discord.Interaction, discord_id: str, никнейм: str):
    try: disc_id = int(discord_id)
        ...
        await interaction.response.send_message("❌ Неверный формат ID.", ephemeral=True); return
    никнейм = никнейм.replace("_", " ")
    c.execute("SELECT * FROM family_members WHERE discord_id=?", (disc_id,))
    if c.fetchone():
        await interaction.response.send_message(f'⚠️ Пользователь с ID `{disc_id}` уже в семье.', ephemeral=True); return
    c.execute("SELECT * FROM family_members WHERE nickname=?", (никнейм,))
    if c.fetchone():
        await interaction.response.send_message(f'⚠️ Ник `{никнейм}` уже занят.', ephemeral=True); return
...
async def remove_family(interaction: discord.Interaction, discord_id: str):
    ...
    row = c.fetchone()
    if not row:
        await interaction.response.send_message(f'❌ Пользователь с ID `{disc_id}` не найден в семье.', ephemeral=True); return
    nickname = row[0]
...
async def family_list(interaction: discord.Interaction):
    ...
    rows = c.fetchall()
    if not rows:
        await interaction.response.send_message('👪 Семья пуста.', ephemeral=True); return
...
async def add_car(interaction: discord.Interaction, модель: str, госномер: str):
    ...
    if not nick:
        await interaction.response.send_message('❌ Вы не привязаны к семье. Сначала добавьте себя через /дсемья.', ephemeral=True); return
    nick = nick.replace("_", " ")
    try:
...
async def remove_car(interaction: discord.Interaction, госномер: str):
    ...
    if c.rowcount == 0:
        await interaction.response.send_message(f'❌ Машина с госномером `{госномер}` не найдена.', ephemeral=True); return
    conn.commit()
...
async def car_list(interaction: discord.Interaction):
    ...
    cars = c.fetchall()
    if not cars:
        await interaction.response.send_message('🚫 Нет машин.', ephemeral=True); return
    lines = []
...
async def take_car(interaction: discord.Interaction, номер: int, часы: float = 2.0):
    nick = get_member_nick(interaction.user.id)
    if not nick:
        await interaction.response.send_message('❌ Вы не привязаны к семье.', ephemeral=True); return
    nick = nick.replace("_", " ")
    auto_return()
    c.execute("SELECT status, plate FROM vehicles WHERE id=?", (номер,))
    car = c.fetchone()
    if not car:
        await interaction.response.send_message(f'❌ Авто с номером `{номер}` не найдено.', ephemeral=True); return
    status, plate = car
    if status != 'свободен':
        await interaction.response.send_message(f'❌ Авто `{plate}` уже занято.', ephemeral=True); return
    now = datetime.datetime.now()
...
async def return_car(interaction: discord.Interaction, номер: int):
    ...
    plate, status = car
    if status == 'свободен':
        await interaction.response.send_message(f'❌ Авто `{plate}` уже свободно.', ephemeral=True); return
...
async def warehouse_put(interaction: discord.Interaction, предмет: str, категория: str, количество: int):
    ...
    nick = nick.replace("_", " ")
    if количество <= 0:
        await interaction.response.send_message('❌ Количество > 0.', ephemeral=True); return
...
async def warehouse_show(interaction: discord.Interaction, категория: str = "all"):
    if категория == "all":
        ...
        rows = c.fetchall()
        if not rows:
            await interaction.response.send_message('📦 Склад пуст.', ephemeral=True); return
        cats = {}
...
async def warehouse_take(interaction: discord.Interaction, предмет: str, количество: int):
    ...
    if not row or row[0] < количество:
        await interaction.response.send_message(f'❌ Недостаточно `{предмет}` на складе.', ephemeral=True); return
...
async def bank_add(interaction: discord.Interaction, сумма: int, причина: str = ""):
    if сумма <= 0:
        await interaction.response.send_message('❌ Сумма должна быть положительной.', ephemeral=True); return
    nick = get_member_nick(interaction.user.id)
    if not nick:
        await interaction.response.send_message('❌ Вы не привязаны к семье. Используйте /дсемья.', ephemeral=True); return
    nick = nick.replace("_", " ")
...
async def bank_remove(interaction: discord.Interaction, сумма: int, причина: str = ""):
    ...
    balance = get_family_balance()
    if balance < сумма:
        await interaction.response.send_message(f'❌ Недостаточно средств. Баланс: {balance}.', ephemeral=True); return
    c.execute("UPDATE bank SET balance = balance - ?", (сумма,))
    conn.commit()
    new_balance = get_family_balance()
...
async def take_contract(interaction: discord.Interaction, участники: str, название: str, дата: str, время: str, векселя: int = 0):
    ...
    if not members:
        await interaction.response.send_message("❌ Укажите участников через @.", ephemeral=True); return
    try: due_dt = datetime.datetime.strptime(f"{дата} {время}", "%d.%m.%Y %H:%M")
    except ValueError:
        await interaction.response.send_message("❌ Неверный формат даты/времени.", ephemeral=True); return
    ...
    if status_channel is None:
        await interaction.response.send_message("❌ Канал уведомлений о статусе не найден.", ephemeral=True); return
...
async def dv_add(interaction: discord.Interaction, участники: str, тип: str, причина: str):
    ...
    if blocked:
        await interaction.response.send_message(f"❌ Нельзя выдать ДВ следующим участникам (Отпуск): {', '.join(blocked)}", ephemeral=True); return
...
async def dv_remove(interaction: discord.Interaction, участник: str, причина: str):
    ...
    if not uid or not uid.isdigit():
        await interaction.response.send_message("❌ Укажите участника через @.", ephemeral=True); return
    member = interaction.guild.get_member(int(uid))
    if member is None:
        await interaction.response.send_message("❌ Участник не найден.", ephemeral=True); return
    ...
    if row:
        ...
        if last_dv_date and (datetime.datetime.now() - last_dv_date).days < 7:
            ...
            await interaction.response.send_message(f"❌ С момента последнего взыскания прошло менее 7 дней. Осталось: {days_left} дн.", ephemeral=True); return
    ...
    if c.rowcount == 0:
        await interaction.response.send_message(f'❌ У {member.mention} нет выговоров.', ephemeral=True); return
    conn.commit()
...
async def dv_list(interaction: discord.Interaction, участник: str = None):
    if участник:
        ...
        if not nickname:
            await interaction.response.send_message('❌ Укажите @участника или будьте в семье.', ephemeral=True); return
    c.execute("SELECT action_type, reason, issued_by, date FROM disciplinary_actions WHERE nickname=? ORDER BY date DESC", (nickname,))
    rows = c.fetchall()
    if not rows:
        await interaction.response.send_message(f'✅ У `{nickname}` нет выговоров.', ephemeral=True); return
...
async def show_logs(interaction: discord.Interaction, участник: str = None):
    ...
    rows = c.fetchall()
    if not rows:
        await interaction.response.send_message("📋 Логов нет.", ephemeral=True); return
