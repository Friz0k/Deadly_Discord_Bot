...
from config import ROLE_FAMILY, ROLE_BANK_OWNER, ROLE_BANK_MANAGER
...
class BankCog(commands.Cog):
    ...
    async def topup(self, interaction: discord.Interaction, amount: int, reason: str):
        if not self.has_role(interaction, ROLE_FAMILY):
            await interaction.response.send_message("❌ Нет прав.", ephemeral=True)
            return
        if amount <= 0:
        ...
        if not interaction.attachments:
            await interaction.response.send_message("❌ Прикрепите скриншот подтверждения.", ephemeral=True)
            return
    ...
    async def withdraw(self, interaction: discord.Interaction, user: discord.User, amount: int, reason: str = None):
        if not (self.has_role(interaction, ROLE_BANK_OWNER) or self.has_role(interaction, ROLE_BANK_MANAGER)):
            await interaction.response.send_message("❌ Нет прав.", ephemeral=True)
            return
        if amount <= 0:
            await interaction.response.send_message("❌ Сумма должна быть положительной.", ephemeral=True)
